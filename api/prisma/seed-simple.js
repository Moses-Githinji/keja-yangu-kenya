  ...
  for (const location of kenyaLocations) {
    for (const area of location.areas) {
      ...
      for (let i = 0; i < numProperties; i++) {
        ...
        // Generate amenities (subset of features)
        const amenities = features.slice(0, Math.min(5, features.length));
          ...
          for (let j = 0; j < numImages; j++) {
            await prisma.propertyImage.create({
              data: {
                ...
                altText: j === 0 ? "Main view" : `Property view ${j + 1}`,
                isPrimary: j === 0,
                order: j,
              },
            });
...
async function main() {
  ...
  try {
    ...
    console.log(`   - Property Images: ${properties.length * 5} (average)`);
  } catch (error) {
